...
- **[Part 1: Authorization models](/programming/authorization-models.html)** — the PIP and the PAP. The access matrix and its two projections, why DAC and MAC are answers to the mutation question rather than rungs of a ladder, and the five separable concerns that most authorization arguments confuse for one. Ends with the four questions every invocation answers on its way to an effect — acquire, designate, reach, authorize — which the rest of the series uses as axes.
...
- **[Part 3: How authority is enforced](/programming/authority-enforcement.html)** — the PEP. The reference monitor and its three properties. Each of the four axes has its own enforcer, and unnameability versus adjudication is a choice of which axis to cut. Then why Linux is a toolkit rather than a primitive, and what can change between the check and the use.
