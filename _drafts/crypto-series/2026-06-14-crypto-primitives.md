...
In the formal vocabulary: bare confidentiality only buys you IND-CPA (security against a passive eavesdropper). An attacker who can *submit* ciphertexts and watch what happens demands IND-CCA — and authenticated encryption is exactly how you get there (AE implies IND-CCA). So "use an AEAD" is the one-line engineering answer to the entire chosen-ciphertext family — padding oracles, POODLE, and friends. You don't need a separate IND-CCA checkbox; a correctly-used AEAD *is* the checkbox.

**What AEAD still doesn't give you: key-commitment.** Here's the surprising gap, and the one that bites real systems. AEAD promises confidentiality and integrity — but *not* that a ciphertext is bound to a single key. AES-GCM and ChaCha20-Poly1305 are *non-committing*: an attacker can craft one ciphertext that decrypts and verifies successfully under several different keys. Nobody required otherwise, so nobody got it. This breaks any design that reads "the tag verified" as "this was the right key" — it's the engine behind **partitioning-oracle attacks** (which broke Shadowsocks proxies and accelerate password/PAKE guessing), and it bites password-based encryption, multi-recipient and key-rotation schemes, JWE, and age. The fix is a **committing AEAD** (a key-committing transform layered over the usual modes). Related sharp edge: **RUP** — release of unverified plaintext — if an implementation emits decrypted bytes *before* checking the tag (tempting in streaming / large-file APIs), it hands plaintext to exactly the active attacker AEAD was meant to stop. Both are reminders that "authenticated encryption" is a precise guarantee — confidentiality and integrity, and it stops there — not a general-purpose seal of safety.
...
3. [Partitioning Oracle Attacks - Len, Grubbs & Ristenpart (USENIX '21)][partitioning-oracle]
4. [How to Abuse and Fix Authenticated Encryption Without Key Commitment - Albertini et al. (USENIX '22)][key-commitment]
...
[partitioning-oracle]: https://www.usenix.org/conference/usenixsecurity21/presentation/len "Partitioning Oracle Attacks - Len, Grubbs & Ristenpart (USENIX Security 2021)"
[key-commitment]: https://www.usenix.org/conference/usenixsecurity22/presentation/albertini "How to Abuse and Fix Authenticated Encryption Without Key Commitment - Albertini, Duong, Gueron, Kölbl, Luykx, Schmieg (USENIX Security 2022)"
